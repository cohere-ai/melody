...
func TestParseVisionGeneration(t *testing.T) {
	...
	if gen.Segments[0].Kind != "element" || gen.Segments[0].Element == nil {
	...
	if gen.Segments[1].Kind != "text" || !strings.Contains(gen.Segments[1].Text, "Figure 2") {
...
func TestParseVisionGenerationProseOnly(t *testing.T) {
	...
	if len(gen.Segments) != 1 || gen.Segments[0].Kind != "text" {
		t.Fatalf("%+v", gen)
	}
}
