...
/// The values produced by the cmd5 template are simple identifiers / numbers
/// / booleans and never contain a literal `"`, so a naive scan is safe.
...
impl FilterImpl {
    ...
    fn handle_cofl_before_tool_call(&mut self, s: &str) -> (Vec<FilterOutput>, usize) {
        ...
        let id = extract_attr(attrs, "id").unwrap_or("").to_string();
        let name = extract_attr(attrs, "name").unwrap_or("").to_string();
    ...
    fn handle_cofl_in_tool_param_value(&mut self, s: &str) -> (Vec<FilterOutput>, usize) {
        ...
        if is_final {
            self.cofl_action_metadata.mode = CoflMode::InToolCallBody;
            let (more, r) = self.parse_cofl_actions(&s[consumed..]);
            out.extend(more);
            (out, consumed + r)
        } else {
            (out, consumed)
    ...
    fn emit_cofl_param_value_chunk(&mut self, chunk: &str, is_final: bool) -> Vec<FilterOutput> {
        ...
        let mut delta = if self.cofl_action_metadata.cur_param_is_string {
            json_escape_string_content(chunk)
        } else {
            chunk.to_string()
        };
